...
def run_nas_algorithm(algo_params, search_space, mp):
    ...
    # Adding BOSHNAS here
    elif algo_name == 'boshnas':
        data = boshnas(search_space, **ps)
...
def boshnas(search_space, 
    ...
    Heteroscedastic surrogate models for Neural Architecture Search
    
    Args:
        search_space (Nasbench): Nasbench class object
        num_init (int, optional): initial seed points in the design
            space to start with
        k (int, optional): number of architectures added to the dataset
            with lowest acquisition function
        loss (TYPE, optional): type of loss to minimize; one in 
            ['val_loss', 'test_loss']
        total_queries (int, optional): total number of queries allowed
            for the algorithm
        acq_opt_type (str, optional): algorithm to get new architectures;
            one in ['mutation', 'mutation_random', 'random']
        num_arches_to_mutate (int, optional): number of top architectures
            to perform mutation on
        max_mutation_rate (int, optional): maximum mutation rate
        ...
        predictor_encoding (str, optional): type of encoding for the 
            input to the predictor; one in ['adj', 'bohamiann', 'act_adj', 
            'cont_adj', 'path', 'trunc_path', 'cat_path', 'trunc_cat_path',
            'gcn', 'vae']
        cutoff (int, optional): used for truncation of path encoding
        mutate_encoding (str, optional): type of mutation algorithm; one
            in ['adj', 'cont_adj', 'trunc_path', 'cat_path', 'trunc_cat_path',
            'wtd_trunc_path' ...]
        random_encoding (str, optional): type of random generation; one 
            in ['constrained', 'uniforms', 'adj', 'cont_adj', 'trunc_path',
            'cat_path', 'trunc_cat_path', 'wtd_trunc_path' ...]
        implement_gobi (bool, optional): next query generated by GOBI algorithm
        trust_region (bool, optional): if True, queries searched within
            a trust region
    ...
    Returns:
        data: output data
    """
    # TODO: implement CNN2vec / Transformer2vec graph encodings here

    # Get an initial randomly generated dataset seed for training the surrogate function
    data = search_space.generate_random_dataset(num=num_init, 
                                                predictor_encoding=predictor_encoding, 
                                                random_encoding=random_encoding,
                                                deterministic_loss=deterministic,
    ...
    while query <= total_queries:
        ...
        if implement_gobi:
            ...
            # current data. This keeps number of queries limited, of which argmin of
            # acquisition function is easily determined
            
            # Get a set of candidate architectures by mutation
            candidates = search_space.get_candidates(data,
                                                     acq_opt_type=acq_opt_type,
                                                     predictor_encoding=predictor_encoding, 
                                                     mutate_encoding=mutate_encoding,
            ...
            candidate_predictions, uncertainties = meta_neuralnet.predict(xcandidates)

            # compute the acquisition function for all the candidate architectures
            candidate_indices = acq_fn(candidate_predictions, ytrain=ytrain, stds=uncertainties, explore_type=explore_type)
